...
flags.DEFINE_string(
    "vocab_path", None, "Vocabulary path for query and document tokens."
)
...
flags.DEFINE_integer(
    "list_size", None, "List size used for training. Use None for dynamic list size."
)
flags.DEFINE_integer("group_size", 1, "Group size used in score function.")
flags.DEFINE_string(
    "loss", "approx_ndcg_loss", "The RankingLossKey for the loss function."
)
flags.DEFINE_string(
...
def _get_example_weight_feature_column():
    if FLAGS.weights_feature_name:
        return tf.feature_column.numeric_column(
            FLAGS.weights_feature_name, dtype=tf.float32, default_value=1.0
        )
    return None


def make_input_fn(file_pattern, batch_size, randomize_input=True, num_epochs=None):
