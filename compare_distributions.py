...
# neuer param plot run über alle parameter
file = "200er_full_results.pickle"

# skewed results, containing the data from the first highly optimized run, focussing only on "good" param combinations
#  file = "new_results.pickle"
...
df["acc_auc"] = df["acc_auc"].multiply(100)
...
def find_best_distribution(param, save=False, one_vs_rest_params=False):
    ...
    sel2 = df.loc[df["true_weak?"] == False]["acc_auc"]
    selections.append((sel2, "No Weak: " + str(len(sel2))))
    for s in subsets:
        sel1 = df.loc[df[param].isin(s) & df["true_weak?"] == True]["acc_auc"]
...
def find_multiple_hyper_param_combinations(params):
    baseline = df.loc[df["true_weak?"] == False]["acc_auc"]
...
def get_distributions_for_interesting(params):
    baseline = df.loc[df["true_weak?"] == False]["acc_auc"]
    true_interesting = df.loc[df["interesting?"] == True]["acc_auc"]
    false_interesting = df.loc[df["interesting?"] == False]["acc_auc"]
    highest_diff = calculate_difference(true_interesting, baseline)
    compare_two_distributions(
        [
            (
                baseline,
                "No Weak Supervision ({}):         {:.3}%".format(
                    len(baseline), baseline.mean()
                ),
            ),
            (
                true_interesting,
                "Weak and improvement ({}):        {:.3}%".format(
                    len(true_interesting), true_interesting.mean()
                ),
            ),
            (
                false_interesting,
                "Weak and no improvement ({}): {:.3}%".format(
                    len(false_interesting), false_interesting.mean()
                ),
            ),
        ],
        axvline=True,
        #  title='Difference mean "Weak and improvement" to "No Weak ":{:.3}%, mean including 95% confidence interval'.format(
        #  highest_diff
        #  ),
        save=True,
    )
    ...
    for param in params[0]:
        ...
        for value in param_dist[param.upper()]:
            sel = true_interesting.loc[true_interesting[param] == value]["acc_auc"]
            selections.append(
    ...
    #  true_interesting["acc_auc"] = true_interesting["acc_auc"].multiply(100)
...
hyper_test_params = [
    [
        ...
        "uncertainty_recommendation_certainty_threshold",
        "cluster_recommendation_minimum_cluster_unity_size",
        #  "minimum_test_accuracy_before_recommendations", <<<---- wird ni in der Datenbank gespeichert -,- FIXME
        #  "uncertainty_recommendation_ratio",
    ],
    #  [
    #      "sampling",
