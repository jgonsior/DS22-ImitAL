...
def calc_hypersphere_intersection_volume(
    center_1: List[float], r_1: float, center_2: List[float], r_2: float, n: int
) -> float:
    """
    ...
    if d >= r_1 + r_2:
        ...
        return calc_hypersphere_cap_volume(r_1, c_1, n) + calc_hypersphere_cap_volume(
            r_2, c_2, n
        )
...
def _print_iou_matrix(
    hs: List[Tuple[float, List[float]]], m_rel: int, hyperspheres: bool
) -> None:
    """
    ...
    for i, h_i in enumerate(hs):
        for j, h_j in enumerate(hs):
            if i <= j:
                continue
            result[i][j] = (
                calc_hypersphere_iou([h_i], h_j, m_rel)[0]
                if hyperspheres
                else approximate_hypercube_iou([h_i], h_j, m_rel)[0]
            )

    print(result)


def approximate_hypercube_iou(
    hs: List[Tuple[float, List[float]]], new_hs: Tuple[float, List[float]], m_rel: int
) -> List[float]:
    """
...
def check_iou_threshold(
    ...
    if iou_threshold is None:
        return True
    ious = (
        np.array(calc_hypersphere_iou(hs, (r, c_i), m_rel))
        if hyperspheres
        else np.array(approximate_hypercube_iou(hs, (r, c_i), m_rel))
    )
    print(ious)
    if np.all(ious == -1):
        return True
...
def add_redundant(
    dataset: pd.DataFrame, m_red: int, random_state: int = None
) -> pd.DataFrame:
    """
...
def add_noise_multilabel(
    labels: pd.DataFrame, noise_levels: Union[List[float], None], q: int
) -> List[pd.DataFrame]:
    """
    ...
    if noise_levels is not None and noise_levels:
        for i, noise in enumerate(noise_levels):
            ...
            noisy_labels.append(
                pd.DataFrame(new_labels, columns=["l{}".format(i) for i in range(q)])
            )

    return noisy_labels


def add_noise_singlelabel(
    labels: pd.DataFrame, noise_levels: Union[List[float], None], q: int
) -> List[pd.DataFrame]:
    """
